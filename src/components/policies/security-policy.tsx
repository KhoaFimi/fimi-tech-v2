	...
	return (
		<div className='flex flex-col gap-2 text-justify text-xs'>
			...
			<p>
				...
				hợp với quy định của pháp luật (kể cả Nghị định số 13/2023/NĐ-CP ngày 17
				tháng 4 năm 2023 về bảo vệ dữ liệu cá nhân như được sửa đổi, bổ sung
				hoặc thay thế tùy từng thời điểm), Thông Báo Bảo Mật này và các điều
				khoản về sự riêng tư trong (các) thỏa thuận với chúng tôi.
			</p>
			<p>
				Thông Báo Bảo Mật này là một phần của các điều khoản và điều kiện tương
				ứng điều chỉnh mối quan hệ giữa Quý khách và chúng tôi, bao gồm nhưng
				không giới hạn ở, như có thể được áp dụng, Điều Khoản và Điều Kiện của
				các sản phẩm Thẻ Tín Dụng được phát hành bởi các Tổ chức tín dụng đối
				tác của FIMI, Điều khoản và Điều kiện Sử Dụng các nền tảng của FIMI (gọi
				chung là “Các Điều Khoản và Điều Kiện”) và nên được đọc cùng với Các
				Điều Khoản Và Điều Kiện đó. Bằng việc đăng ký sử dụng hoặc sử dụng các
				sản phẩm hoặc dịch vụ của chúng tôi hoặc bằng việc cho phép chúng tôi sử
				dụng thông tin cá nhân của Quý khách, Quý khách chấp nhận các chính sách
				và thực tiễn được mô tả trong Thông Báo Bảo Mật này.
			</p>
			<div className='flex flex-col gap-2'>
				...
				<ul className='space-y-1 px-2'>
					...
					<li>
						- Các lựa chọn mà chúng tôi đề xuất, kể cả cách thức truy cập và cập
						nhật dữ liệu cá nhân
					</li>
				</ul>
				<p>
					...
					người liên hệ trong các trường hợp khẩn cấp hoặc cá nhân khác của Quý
					khách) cho chúng tôi, Quý khách cam đoan và bảo đảm rằng Quý khách đã
					có được sự đồng ý của bên thứ ba đó cho việc xử lý (bao gồm nhưng
					không giới hạn việc thu thập, sử dụng và tiết lộ) các thông tin cá
					nhân cho các mục đích được nêu tại văn bản này.
				</p>
			</div>
			<p>
				...
				suốt quá trình chúng tôi giao dịch với Quý khách. Điều này có nghĩa rằng
				chúng tôi quyết định mục đích và phương tiện xử lý dữ liệu cá nhân và
				khi hành động với tư cách là bên xử lý dữ liệu cá nhân, chúng tôi sẽ
				trực tiếp xử lý dữ liệu cá nhân đó.
			</p>
			<div className='flex-2 flex flex-col gap-2'>
				...
				<p>
					1.1 Để chúng tôi có thể xử lý các yêu cầu của Quý khách, có thể mở và
					điều hành tài khoản/tiện ích cho Quý khách và/hoặc nhằm cung cấp khái
					quát các sản phẩm và dịch vụ của chúng tôi, chúng tôi, và bên xử lý dữ
					liệu mà chúng tôi thuê, có thể cần phải và/hoặc được yêu cầu phải thu
					thập, ghi, phân tích, xác nhận, lưu trữ, chỉnh sửa, sử dụng, công
					khai, kết hợp, truy cập, truy xuất, thu hồi, mã hóa, giải mã, sao
					chép, chia sẻ, truyền đưa, cung cấp, chuyển giao, xóa, hủy dữ liệu cá
					nhân hoặc các hành động khác có liên quan (nghĩa là, “xử lý”) dữ liệu
					cá nhân, mà các dữ liệu này chứa đựng bất kỳ dữ liệu nào có liên quan
					một cách trực tiếp hoặc gián tiếp đến Quý khách và/hoặc các giao dịch
					mà Quý khách thực hiện với chúng tôi. Chúng tôi, và bên xử lý dữ liệu
					mà chúng tôi thuê, có thể xử lý các loại dữ liệu cá nhân, bao gồm cả
					dữ liệu cá nhân nhạy cảm, được thu thập sau đây về Quý khách:
				</p>
				{info1.map((data, i) => (
					<p key={i}>{data}</p>
				))}
				<p>
					...
					thập những dữ liệu này từ Quý khách khi Quý khách yêu cầu, hoặc trong
					quá trình chúng tôi cung cấp cho Quý khách, bất kỳ sản phẩm và dịch vụ
					nào của chúng tôi, và từ nhiều nguồn đa dạng, bao gồm nhưng không giới
					hạn: <br /> (i) thông qua quan hệ giữa Quý khách và chúng tôi, ví dụ
					thông tin mà Quý khách cung cấp trong các văn bản đăng ký hoặc các tài
					liệu hỗ trợ giao dịch, khi Quý khách sử dụng các sản phẩm hoặc dịch vụ
					của chúng tôi, khi tham gia các khảo sát, sự cạnh tranh và chương
					trình khuyến mại dành cho khách hàng, và trong quá trình đánh giá tài
					chính; <br /> (ii) thông qua liên hệ trao đổi bằng lời nói và văn bản
					của Quý khách với chúng tôi và/ hoặc các đối tác hợp tác kinh doanh
					của chúng tôi; <br /> (iii) từ các nhà cung cấp, bên cung cấp dịch vụ,
					đối tác, thương nhân và các bên thứ ba, bao gồm nhưng không giới hạn
					các bên tư vấn về khảo sát, truyền thông mạng xã hội, tiếp thị, tham
					chiếu tín dụng, ngăn ngừa gian lận, tập hợp dữ liệu; bên hỗ trợ cơ sở
					hạ tầng và phương tiện, và các bên thứ ba khác liên quan đến hoạt động
					kinh doanh của chúng tôi; <br /> (iv) từ bất kỳ bên cung cấp báo cáo
					tín dụng, bên cung cấp tham chiếu tín dụng và các cơ quan thuộc chính
					phủ, hoặc thông tin thu thập từ bất kỳ nguồn công cộng sẵn có, các
					hướng dẫn, hoặc cơ quan đăng ký nào; <br /> (v) từ hoạt động phân tích
					Quý khách sử dụng và quản lý (các) tài khoản/ tiện ích của Quý khách
					tại nền tảng của chúng tôi (bao gồm trang web FIMI.tech và Ứng dụng
					FIMI), từ các giao dịch mà Quý khách thực hiện và từ khoản thanh toán
					được thực hiện từ/ đến (các) tài khoản/ tiện ích của Quý khách;
				</p>
				{info2.map((data, i) => (
					<p key={i}>{data}</p>
				))}
				<p>
					1.3 Chúng tôi và những người đại diện của chúng tôi có thể ghi lại và/
					hoặc quản lý các hệ thống thông tin liên lạc (bao gồm điện thoại, các
					hội thoại qua điện thoại cố định và điện thoại di động, thư điện tử,
					tin nhắn tức thời, phòng trò chuyện, fax và các phương tiện liên lạc
					điện tử khác) giữa Quý khách, đại diện theo ủy quyền hoặc được cho
					phép, với nhân viên, đại lý, bên tư vấn của FIMI Chúng tôi chỉ thực
					hiện những hoạt động nêu trên trong phạm vi pháp luật áp dụng cho phép
					nhằm phục vụ những mục đích hợp pháp hoặc những mục đích mà pháp luật
					cho phép hoặc yêu cầu. Chúng tôi thu thập dữ liệu cá nhân này vì một
					số lý do bao gồm nhưng không giới hạn:
				</p>
				{info3.map((data, i) => (
					<p key={i}>{data}</p>
				))}
				...
				<p>
					2.1 Chúng tôi, và bên xử lý dữ liệu mà chúng tôi sử dụng, có thể xử lý
					dữ liệu cá nhân vì một hoặc nhiều mục đích sau đây:
				</p>
				{info4.map((data, i) => (
					<p key={i}>{data}</p>
				))}
				<p>
					...
					(cookie) là một tệp văn bản nhỏ mà một trang mạng có thể gửi đến trình
					duyệt của Quý khách, mà sau đó có thể được lưu trữ trên hệ thống của
					Quý khách. Chúng tôi sử dụng cookies trong một số trang của chúng tôi
					để thu thập thông tin về những người dùng trang mạng của chúng tôi (ví
					dụ, lưu lại các tùy chọn của những người dùng và ghi lại thông tin
					phiên truy cập) và thông tin mà chúng tôi thu thập sẽ được sử dụng sau
					đó để đảm bảo mức độ dịch vụ được cá nhân hóa cho những người dùng của
					chúng tôi. Quý khách có thể điều chỉnh các cài đặt trên trình duyệt
					của Quý khách để Quý khách được thông báo khi nhận một cookie. Trong
					trường hợp Quý khách mong muốn tắt các cookies được liên kết với các
					công nghệ đó, Quý khách có thể làm điều đó bằng cách thay đổi các cài
					đặt trong trình duyệt của Quý khách. Tuy nhiên, khi làm như vậy, Quý
					khách có thể không thể sử dụng một số chức năng nhất định hoặc truy
					cập và một số (các) phần nhất định trên các trang mạng của chúng tôi.
				</p>
				<p>
				...
				<p>
					3.1 Để cung cấp cho Quý khách các sản phẩm và dịch vụ của chúng tôi và
					để quản lý và/hoặc vận hành các sản phẩm và dịch vụ đó, và/hoặc để
					phục vụ các mục đích khác như được mô tả ở trên, chúng tôi có thể tiết
					lộ dữ liệu cá nhân của Quý khách hoặc dữ liệu cá nhân của các bên thứ
					ba có liên quan đến Quý khách, cho các bên dưới đây:
				</p>
				{info5.map((data, i) => (
					<p key={i}>{data}</p>
				))}
				...
				<p>
					3.3 Mặt khác, chúng tôi sẽ coi dữ liệu cá nhân của Quý khách là riêng
					tư và bí mật và ngoài các bên đã nêu ở trên, chúng tôi sẽ không tiết
					lộ dữ liệu của Quý khách cho bất kỳ bên nào khác ngoại trừ:
				</p>
				{info6.map((data, i) => (
					<p key={i}>{data}</p>
				))}
				...
				<p>
					...
					xử lý dữ liệu không phù hợp. Hiểu được tầm quan trọng của việc bảo vệ
					dữ liệu cá nhân, chúng tôi coi thông tin là tài sản quý giá của mình
					và do đó chúng tôi rất coi trọng việc đảm bảo an toàn đối với dữ liệu
					cá nhân của Quý khách. Chúng tôi thường xuyên xem xét và cập nhật các
					biện pháp quản lý và kỹ thuật khi xử lý dữ liệu cá nhân của Quý khách.
				</p>
				<p>
					4.2. Nhân viên của chúng tôi được đào tạo để xử lý dữ liệu cá nhân một
					cách an toàn và với sự tôn trọng tối đa, và nếu vi phạm sẽ bị xử lý kỷ
					luật.
				</p>
				<p className='mt-2 font-bold'>
				...
				<p>
					5.2. Khi Quý khách muốn truy cập vào dữ liệu cá nhân của Quý khách mà
					chúng tôi đang sở hữu, hoặc khi Quý khách cho rằng dữ liệu cá nhân
					được chúng tôi nắm giữ là không chính xác, không đầy đủ, gây hiểu nhầm
					hoặc không cập nhật, nếu có liên quan, Quý khách có thể yêu cầu chúng
					tôi thông qua các kênh liên hệ chính thống và công khai của chúng tôi
					hoặc qua (các) kênh khác mà chúng tôi có thể thiết lập tùy từng thời
					điểm.
				</p>
				<p>
					...
					khi nhận được yêu cầu hoàn chỉnh và hợp lệ và phí xử lý liên quan (nếu
					có) từ Quý khách, tùy thuộc vào quyền của chúng tôi được viện đến bất
					kỳ sự miễn trừ và/hoặc ngoại lệ nào theo quy định pháp luật.
				</p>
				<p>
					...
					như khi chúng tôi không thể xác nhận danh tính của Quý khách hoặc khi
					dữ liệu được yêu cầu có tính chất thương mại bí mật hoặc trong trường
					hợp chúng tôi nhận được nhiều yêu cầu lặp lại đối với cùng một dữ
					liệu. Tuy nhiên, chúng tôi sẽ thông báo cho Quý khách về lý do không
					thể đồng ý với yêu cầu của Quý khách.
				</p>
				<p>
					5.5. Cũng xin lưu ý rằng, chúng tôi có thể, bằng quyết định của mình,
					cho phép chỉnh sửa theo yêu cầu và/hoặc có thể yêu cầu thêm tài liệu
					chứng minh về dữ liệu mới để tránh gian lận và không chính xác.
				</p>
				<p>
				...
				<p>
					...
					tất cả các mục đích được nêu trong Thông Báo này bằng cách để lại yêu
					cầu qua các kênh liên hệ chính thống và công khai của chúng tôi. Nếu
					Quý khách rút lại sự đồng ý của mình đối với bất kỳ hoặc tất cả các
					mục đích và tuỳ thuộc vào bản chất yêu cầu của Quý khách, FIMI có thể
					sẽ không tiếp tục cung cấp các sản phẩm hoặc dịch vụ của chúng tôi đến
					Quý khách. Việc rút lại sự đồng ý này sẽ được xem là sự chấm dứt từ
					phía Quý khách cho bất kỳ mối quan hệ hợp đồng nào mà Quý khách có với
					chúng tôi, và vi phạm nghĩa vụ hoặc các cam kết theo hợp đồng của Quý
					khách, đồng thời chúng tôi bảo lưu rõ ràng các quyền và biện pháp khắc
					phục hợp pháp của chúng tôi trong những trường hợp đó
				</p>
				<p>
					5.8. Không ảnh hưởng đến những điều trên, Quý khách đồng ý và thừa
					nhận rằng bất kỳ sự rút lại sự đồng ý nào theo các điều khoản được nêu
					trong Thông Báo Bảo Mật này sẽ không ảnh hưởng đến bất kỳ sự đồng ý
					nào mà Quý khách đã có với FIMI đối với việc sử dụng (các) số điện
					thoại ở Việt Nam, các thư điện tử và các dữ liệu cá nhân khác của Quý
					khách để nhận các thông tin tiếp thị hoặc quảng cáo. Tuy vậy Quý khách
					luôn có thể, bằng một yêu cầu cụ thể, rút lại sự đồng ý về việc nhận
					các thông tin tiếp thị nêu trên vào bất kỳ thời điểm nào.{' '}
				</p>
				<p>
					5.9. Thông Báo Bảo Mật này không được xem là vi phạm Điều Khoản và
					Điều Kiện. Các quyền của chúng tôi theo Thông Báo Bảo Mật này được xem
					là bổ sung và không ảnh hưởng đến các quyền khác về thu thập, sử dụng,
					tiết lộ, và các hoạt động xử lý dữ liệu cá nhân khác được nêu trong
					Điều Khoản và Điều Kiện hoặc theo pháp luật hiện hành quy định và
					không một điều khoản nào trong Thông Báo Bảo Mật này hàm ý hạn chế bất
					kỳ quyền nào trong số các quyền khác này của chúng tôi.
				</p>
				<p className='mt-2 font-bold'>Lưu trữ dữ liệu cá nhân của Quý khách</p>
				<p>
					Chúng tôi sẽ lưu trữ dữ liệu cá nhân của Quý khách trong khoảng thời
					gian được yêu cầu trong mối quan hệ với Quý khách, nhằm tuân thủ Thông
					Báo Bảo Mật này và/ hoặc các điều khoản và điều kiện trong (các) thoả
					thuận giữa Quý khách và chúng tôi, trong khoảng thời gian cần thiết
					khác để đáp ứng lợi ích của FIMI và/ hoặc của khách hàng, hoặc khi
					pháp luật và/ hoặc các chính sách có liên quan của FIMI quy định, hoặc
					trong khoảng thời gian khác mà các mục đích cho việc thu thập dữ liệu
					cá nhân của Quý khách vẫn còn hiệu lực.
				</p>
				<p className='mt-2'>
					...
					cho chúng tôi là đúng, chính xác và đầy đủ. Quý khách cần cập nhật kịp
					thời tất cả thay đổi đối với các thông tin đã cung cấp cho chúng tôi.
					Trường hợp Quý khách rút lại đồng ý về việc cho phép thu thập, sử dụng
					và tiết lộ thông tin cá nhân của Quý khách theo Thông Báo Bảo Mật này,
					thì việc rút lại đồng ý đó có thể làm cho dịch vụ của chúng tôi bị
					giới hạn, hạn chế, tạm ngừng, hủy bỏ, ngăn cản hoặc bị cấm đoán, tùy
					từng trường hợp. Chúng tôi sẽ không chịu trách nhiệm đối với Quý khách
					cho bất kỳ tổn thất nào phát sinh, và các quyền hợp pháp của chúng tôi
					sẽ được bảo lưu một cách rõ ràng đối với việc giới hạn, hạn chế, tạm
					ngừng, hủy bỏ, ngăn cản, hoặc cấm đoán đó.
				</p>
				<p className='mt-2 font-bold'>8. Sửa đổi Thông Báo Bảo Mật</p>
				<p>
					Thông Báo Bảo Mật này có thể được sửa đổi theo thời gian. Thông báo về
					bất kỳ sự sửa đổi nào sẽ được đăng tải trên trang điện tử của chúng
					tôi và/ hoặc các phương tiện liên lạc khác mà chúng tôi cho là phù
					hợp.
				</p>
				<p className='mt-2 font-bold'>
				...
				<p>
					Trường hợp Quý khách có bất kỳ câu hỏi nào liên quan đến Thông Báo Bảo
					Mật này hoặc cách chúng tôi xử lý dữ liệu cá nhân của Quý khách, vui
					lòng trao đổi với bất kỳ Nhân Viên Dịch Vụ Khách Hàng nào qua các kênh
					liên hệ chính thống và công khai của chúng tôi tại trang web FIMI.tech
					và Ứng dụng FIMI.
				</p>
			</div>
		</div>
