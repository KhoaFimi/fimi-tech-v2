...
import { addLead } from '@/app/(public)/credit/(hhfn)/_actions/add-lead'
...
const LeadForm: FC<{ code: string; product: string }> = ({ code, product }) => {
	...
	const { mutate, isPending } = useMutation({
		mutationFn: async ({
			...
			code: string
			product: string
		}) => await addLead(values, code, product),
		onSuccess: data => {
			if (data?.error) {
				setError(data?.error)
				return
